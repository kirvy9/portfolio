...
set SCRIPT_DIR=%~dp0
REM JMeter 설치 경로를 저장할 변수를 초기화합니다.
set JMETER_HOME=
...
REM 실제로 JMeter 실행파일이 있는지 검증
if not exist "%JMETER_HOME%\bin\jmeter.bat" (
    echo [오류] JMeter 경로가 잘못되었습니다: %JMETER_HOME%
    exit /b 1
)
echo.
...
REM 테스트 파일 존재 확인
REM 설정된 테스트 계획 파일(.jmx)이 실제로 존재하는지 확인합니다.
...
REM 결과 디렉토리 생성
REM 테스트 결과 및 보고서가 저장될 디렉토리가 없으면 새로 생성합니다.
...
REM JMeter 실행 파일 확인
REM Windows 환경을 위해 jmeter.bat 파일 경로를 먼저 확인합니다.
...
REM 선택된 테스트 모드에 따라 해당 스레드 그룹을 활성화하도록 JMeter에 속성을 전달합니다.
...
REM 실행 결과 확인
REM JMeter 실행 명령어의 종료 코드를 확인하여 성공/실패를 판단합니다.
...
REM HTML 보고서 열기
REM 테스트 완료 후 생성된 HTML 보고서를 자동으로 열지 사용자에게 묻습니다.
echo.
...
if /i "%VIEW_REPORT%"=="Y" (
    ...
    if exist "%REPORT_DIR%\index.html" (
        REM 기본 웹 브라우저로 index.html 파일을 엽니다.
